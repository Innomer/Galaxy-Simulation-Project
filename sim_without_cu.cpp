...
void compute_gravity_cpu(Body *bodies, int n, float dt, float softening) {
    for (int i = 0; i < n; i++) {
...
void initialize_bodies(Body *bodies, int n) {
    ...
    for (int g = 0; g < NUM_GALAXIES; g++) {
        ...
        float height = radius * 0.2f * (g % 2 == 0 ? 1 : -1);
...
int main() {
    printf("Starting Galaxy Collision Simulation with %d particles and %d galaxies (CPU Version)\n", N, NUM_GALAXIES);
    ...
    for (int step = 1; step <= STEPS; step++) {
        ...
        compute_gravity_cpu(h_bodies, N, current_dt, SOFTENING);
        ...
        if (step % SAVE_INTERVAL == 0) {
            ...
            printf("Step %d/%d (%.1f%%) - Min dist: %.2f - CPU Time: %.4f sec\n", 
